...
Option Option::parse(std::string const& option_description)
{
	...
	if (argcount) {
		smatch match;
		if (regex_search(options_end, option_description.end(),
				      match,
				      regex{"\\[default: (.*)\\]", regex::icase}))
		{
			val = match[1].str();
		}
	}
...
public:
	...
	static Tokens from_pattern(std::string const& source) {
		...
		std::for_each(sregex_iterator{ source.begin(), source.end(), re_separators },
			      sregex_iterator{},
			      [&](smatch const& match)
			      {
				      // handle anything before the separator (this is the "stuff" between the delimeters)
				      if (match.prefix().matched) {
					      std::for_each(sregex_iterator{match.prefix().first, match.prefix().second, re_strings},
							    sregex_iterator{},
							    [&](smatch const& m)
							    {
...
std::vector<std::string> parse_section(std::string const& name, std::string const& source) {
	...
	std::for_each(sregex_iterator(source.begin(), source.end(), re_section_pattern),
		      sregex_iterator(),
		      [&](smatch const& match)
	{
...
std::vector<Option> parse_defaults(std::string const& doc) {
	...
	for(auto s : parse_section("options:", doc)) {
		...
		std::for_each(sregex_iterator{ s.begin(), s.end(), pattern },
			      sregex_iterator{},
			      [&](smatch const& m)
		{
