...
/*static*/
Matrix
Quaternion::createRotMatrix(const Vector3& axis, mathType angle)
{
...
{
  validate();
  return point + 2.0 * (Point3)cross(_quaternion, ( cross(_quaternion, point) + (Vector3)(_quaternion[3]*point)));
}
...
Quaternion::validate()
{
  ...
  if (_dirty)
  {
    ...
    Vector3 scaledAxis = (_axis.normalized()*(sin(_angle/2.0f)));;
    _quaternion = Vector3(scaledAxis[0], scaledAxis[1], scaledAxis[2], cos(_angle/2.0f));

    double x2 = scaledAxis[0]*scaledAxis[0];
    double y2 = scaledAxis[1]*scaledAxis[1];
    double z2 = scaledAxis[2]*scaledAxis[2];
    double xy = scaledAxis[0]*scaledAxis[1];
    double xz = scaledAxis[0]*scaledAxis[2];
    double yz = scaledAxis[1]*scaledAxis[2];
    double wx = _quaternion[3]*scaledAxis[0];
    double wy = _quaternion[3]*scaledAxis[1];
    double wz = _quaternion[3]*scaledAxis[2];
