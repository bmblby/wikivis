...
Shader::initInContext(unsigned int contextId)
{
  ...
  if (_geometryShaderSource)
  {
    gs  = glCreateShader( GL_GEOMETRY_SHADER_ARB );
  }
  ...
  if (gs)
  {
    ...
    CheckErrorsGL( ("Shader::initInContext() after attaching geometry shader " + _geometryShaderFileName).c_str());
    ...
    for ( params =_programParameter.begin(); params != _programParameter.end(); params++ )
    {
      ...
      CheckErrorsGL( ("Shader::initInContext() after setting program parameter on " + _geometryShaderFileName).c_str());
    }
  }
...
#ifndef GLOOST_SYSTEM_DISABLE_OUTPUT_ERRORS
    std::cerr << std::endl;
    std::cerr << std::endl << "ERROR in Shader::validateShader on SharedResource " << getSharedResourceId() << ":";
    std::cerr << std::endl << "         Compile error(s) in file \"" << shaderFileName << "\" !";
    std::cerr << std::endl;
...
#ifndef GLOOST_SYSTEM_DISABLE_OUTPUT_ERRORS
      std::cerr << std::endl;
      std::cerr << std::endl << "ERROR in shader::loadSourceFile() on SharedResource " << getSharedResourceId() << ":";
      std::cerr << std::endl << "         Can't read shader source file: \"" << fileName << "\"";
      std::cerr << std::endl;
#endif
...
/**
  ...
  \remarks 4. If your type is GL_GEOMETRY_VERTICES_OUT_EXT your parameter specifies the number of output primitives
              wich can be retrieved with glGetIntegerv(GL_MAX_GEOMETRY_OUTPUT_VERTICES, &n);
*/
