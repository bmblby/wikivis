...
Matrix::invert ()
{
  ...
  if(fabs(rDet) < GLOOST_MATHTYPE_MIN_EPSILON)
  {
...
void
Matrix::setTranslate(const Vector3& v)
{
  _data[12] = v[0];
  _data[13] = v[1];
  _data[14] = v[2];
...
void
Matrix::setTranslate(const Point3& p)
{
  _data[12] = p[0];
  _data[13] = p[1];
  _data[14] = p[2];
...
void
Matrix::setScale(const Point3& p)
{
  _data[0]  = p[0];
  _data[5]  = p[1];
  _data[10] = p[2];
...
void
Matrix::setRotate(const Vector3& v)
{
...
void
Matrix::setRotate(const Vector3& components, mathType angle)
{
  setRotate(components[0]*angle, components[1]*angle, components[2]*angle);
}
...
{
//  setIdentity();
  const mathType cosine = cos(angle);
  const mathType sine   = sin(angle);
...
//  setIdentity();
  ...
  const mathType sine   = sin(angle);
  _data[0] = cosine;
  _data[2] =  -sine;   // <-- Bug found on 2011.02.28 changed sign of _data[2] and _data[8]
  _data[8] =  sine;    // because rotation around Y was in wrong direction
  _data[10] = cosine;  // http://en.wikipedia.org/wiki/Rotation_matrix#Rotations_in_three_dimensions
}
...
operator*(const Matrix& m,
    const Vector3& v )
{
  return Vector3(m[0]*v[0] + m[4]*v[1] + m[8] *v[2] + m[12] *v[3],
...
operator*(const Matrix& m,
    const Point3& p )
{
  return Point3(m[0]*p[0] + m[4]*p[1] + m[8] *p[2] + m[12] *p[3],
...
/**
  \brief factory methode creates an identity matrix
  \remarks ...
*/
...
/**
  \brief factory methode creates a translation matrix
  \remarks ...
*/

/*static*/
Matrix Matrix::createTransMatrix(const mathType x, const mathType y, const mathType z)
{
  Matrix mat;
  mat.setIdentity();
...
/*static*/
Matrix Matrix::createTransMatrix(const Vector3& v)
{
  Matrix mat;
  mat.setIdentity();
  mat.setTranslate(v);
...
/**
  \brief   factory methode creates a scale matrix
  \remarks ...
*/

/*static*/
Matrix Matrix::createScaleMatrix(mathType x, mathType y, mathType z)
{
  Matrix mat;
  mat.setIdentity();
  mat.setScale(x,y,z);
...
/*static*/
Matrix Matrix::createScaleMatrix(const Vector3& v)
{
  Matrix mat;
  mat.setIdentity();
  mat.setScale(v);
...
/*static*/
Matrix Matrix::createScaleMatrix(mathType s)
{
  Matrix mat;
  mat.setIdentity();
  mat.setScale(s);
...
/**
  \brief   factory methode creates a rotation matrix
  \remarks ...
*/

/*static*/
Matrix Matrix::createRotMatrix(mathType x, mathType y, mathType z)
{
  Matrix mat;
  mat.setIdentity();
  mat.setRotate(x,y,z);
...
/*static*/
Matrix Matrix::createRotMatrix(const Vector3& v)
{
  Matrix mat;
  mat.setIdentity();
  mat.setRotate(v);
...
/**
  \brief   factory methode creates a rotation matrix around an axis
  \remarks ...
*/

/*static*/
Matrix Matrix::createRotMatrix(const Vector3& axis, mathType angle)
{
