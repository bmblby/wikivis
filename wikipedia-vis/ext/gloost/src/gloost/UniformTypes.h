...
//    a = 0.0;
  ...
  tinyxml2::XMLElement* toXmlElement(const std::shared_ptr<tinyxml2::XMLDocument>& xmlDoc) const
  {
    auto* rootElement = xmlDoc->NewElement("vec4");
    rootElement->SetAttribute("fd_version", "0");
    {
      rootElement->SetAttribute("r", gloost::helper::toString(r).c_str());
      rootElement->SetAttribute("g", gloost::helper::toString(g).c_str());
      rootElement->SetAttribute("b", gloost::helper::toString(b).c_str());
      rootElement->SetAttribute("a", gloost::helper::toString(a).c_str());
    }
    return rootElement;

  }

  bool fromXmlElement(tinyxml2::XMLElement* rootElement)
  {
    if (!rootElement) return false;

    auto fdVersion = 0;
    gloost::helper::interpretFromChar(fdVersion, rootElement->Attribute("fd_version"));
    if (fdVersion == 0)
    {
      r=1.0f; g=0.0f; g=0.0f; a=1.0f;
      gloost::helper::interpretFromChar(r, rootElement->Attribute("r"));
      gloost::helper::interpretFromChar(g, rootElement->Attribute("g"));
      gloost::helper::interpretFromChar(b, rootElement->Attribute("b"));
      gloost::helper::interpretFromChar(a, rootElement->Attribute("a"));
    }
    return true;
  }
